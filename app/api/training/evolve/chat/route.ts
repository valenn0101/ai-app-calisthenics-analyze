...
function formatRoutineContext(routine: Routine): string {
  const lines: string[] = [
    `Nombre: ${routine.name}`,
    `Semanas: ${routine.weekCount}${routine.hasDeload ? " (incluye descarga)" : ""}`,
  ];
  ...
  for (const day of routine.days) {
    ...
    for (const block of day.blocks) {
      ...
      for (const ex of block.exercises) {
        lines.push(
          `    · ${ex.name} — ${ex.setsScheme}${ex.notes ? ` — ${ex.notes}` : ""} [${ex.muscleGroup}${ex.isProgressive ? ", progresivo" : ""}]`,
        );
      }
  ...
  for (const log of logs.slice(-4)) {
    ...
    for (const dayLog of log.days) {
      ...
      for (const blockLog of dayLog.blocks) {
        const routineBlock = routineDay.blocks.find(
          (b) => b.id === blockLog.blockId,
        );
        if (!routineBlock) continue;
        for (const exLog of blockLog.exercises) {
          const routineEx = routineBlock.exercises.find(
          ...
          const setsStr = completedSets
            .map((s) => `${s.weight > 0 ? s.weight + "kg" : "PC"} × ${s.reps}`)
            .join(", ");
